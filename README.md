...
-----
1. Launch *Serenade*.
2. Click into the circle to enable Listening mode. The circle should move when you're speaking into the mic.
...
🔧 *"Windows, install [app]."*
------------------------------
Installs an application from Microsoft Store, replace [app] by: `9 ZIP`, `CrystalDiskInfo`, `Edge`, `Discord`, `Firefox`, `IrfanView`, `Netflix`, `Opera Browser`, `Opera GX`, `One Calendar`, `Paint 3D`, `Spotify`, `Skype`, `TikTok`, `Twitter`, `Visual Studio Code`, `VLC media player`, or `Windows Terminal`. Note that Microsoft Store can automatically install app updates, so they will always be up-to-date.
