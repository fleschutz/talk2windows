...
❔ *"Windows, list [table]"*
---------------------------
Shows the given table, just replace [table] by: `bluetooth devices`, `earthquakes`, `environment variables`, `headlines`, `installed apps`, `installed software`, `installed voices`, `printers`, `running processes`, `services`, `system info`, `tasks`, `time zones`, or `user groups`.
